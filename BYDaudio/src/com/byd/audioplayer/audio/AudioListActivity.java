...
/**
 * 启动Audio List页面，指定default的页面的方法： Intent intent = new Intent();
 * intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
 * intent.setClassName("com.byd.player.audio",
 * "com.byd.player.audio.AudioListActivity"); intent.putExtra("audio_page",
 * number); // number: 0-本地, 1-sdcard, 2-usb, 3-aux, 4-手机 startActivity(intent)
 */
@SuppressLint("ValidFragment")
public class AudioListActivity extends BaseActivity implements
		OnItemClickListener, OnItemLongClickListener, SearchListener,
		DeleteListener {
	...
	private final int[] TAB_IDS = new int[] { R.id.btn_audio_Local,
			R.id.btn_audio_sdcard, R.id.btn_audio_usb, R.id.btn_audio_aux,
			R.id.btn_audio_mobile };
	private final int[] TAB_NORMAL_BGS_ZH = new int[] {
			R.drawable.bg_audio_local_normal, R.drawable.bg_sdcard_normal,
			R.drawable.bg_usb_normal, R.drawable.bg_aux_normal,
			R.drawable.bg_mobile_normal, };
	private final int[] TAB_SELECTED_BGS_ZH = new int[] {
			R.drawable.bg_audio_local_selected, R.drawable.bg_sdcard_selcted,
			R.drawable.bg_usb_selected, R.drawable.bg_aux_selected,
			R.drawable.bg_mobile_selected, };

	private final int[] TAB_NORMAL_BGS_EN = new int[] {
			R.drawable.bg_audio_local_normal_en,
			R.drawable.bg_sdcard_normal_en, R.drawable.bg_usb_normal,
			R.drawable.bg_aux_normal, R.drawable.bg_mobile_normal_en, };
	private final int[] TAB_SELECTED_BGS_EN = new int[] {
			R.drawable.bg_audio_local_selected_en,
			R.drawable.bg_sdcard_selcted_en, R.drawable.bg_usb_selected,
			R.drawable.bg_aux_selected, R.drawable.bg_mobile_selected_en, };
		...
		public void handleMessage(Message msg) {
			...
			case VideoContentObserver.EXTERNAL_VIDEO_CONTENT_CHANGED:
				AudioLoaderManager.getInstance().loadData(
						AudioLoaderManager.EXTERNAL_SDCARD_TYPE);
				AudioLoaderManager.getInstance().loadData(
						AudioLoaderManager.EXTERNAL_USB_TYPE);
				AudioLoaderManager.getInstance().loadData(
						AudioLoaderManager.INTERNAL_TYPE);
				break;
			}
			super.handleMessage(msg);
		}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (startIntent != null && startIntent.hasExtra(TAB_INDEX)) {
			...
			final int tabIndex = getIntent().getIntExtra(TAB_INDEX,
					TAB_INDEX_LOCAL);
			tabIndex(tabIndex);
		...
		delete.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (mAdapter.isEditMode()) {
					FragmentManager fm = AudioListActivity.this
							.getFragmentManager();
					List<Song> songs = mAdapter.getSeletedSongs();
					DeleteDialog.newInstance(AudioListActivity.this, songs)
				...
				if (mAdapter.isSearchMode()) {
					...
					if (!TextUtils.isEmpty(text)) {
						Message msg = mHandler
								.obtainMessage(MSG_SHOW_SEARCH_PROGRESS_DIALOG);
						mHandler.sendMessageDelayed(msg, 2000);
						new AudioSearchTask(AudioListActivity.this,
								AudioListActivity.this).search(text,
								AudioLoaderManager.SEARCH_BY_NAME);
					}
				}
			}
		});
			...
			public void onClick(View v) {
				if (mAdapter.isSearchMode()) {
					...
					if (!TextUtils.isEmpty(text)) {
						...
						new AudioSearchTask(AudioListActivity.this,
								AudioListActivity.this).search(text,
								AudioLoaderManager.SEARCH_BY_SINGER);
					}
				}
			}
		});
	...
	private void setMode(int mode) {
		...
		case MODE_NORMAL:
			if (!mAdapter.isNormalMode()) {
				findViewById(R.id.btn_edit_container).setVisibility(
						View.VISIBLE);
				findViewById(R.id.button_header_search).setVisibility(
						View.VISIBLE);
				findViewById(R.id.button_header_edit).setVisibility(
						View.VISIBLE);
				findViewById(R.id.btn_bottom_container).setVisibility(
						View.VISIBLE);

				findViewById(R.id.button_header_delete)
						.setVisibility(View.GONE);
				findViewById(R.id.button_header_delete_all).setVisibility(
						View.GONE);
				findViewById(R.id.search_text_container).setVisibility(
						View.GONE);
				findViewById(R.id.btn_search_container)
						.setVisibility(View.GONE);
			}
			break;
		case MODE_EDIT:
			if (!mAdapter.isEditMode() && mAdapter.getCount() > 0) {
				findViewById(R.id.btn_edit_container).setVisibility(
						View.VISIBLE);
				findViewById(R.id.button_header_delete).setVisibility(
						View.VISIBLE);
				findViewById(R.id.button_header_delete_all).setVisibility(
						View.VISIBLE);
				findViewById(R.id.btn_bottom_container).setVisibility(
				...
				findViewById(R.id.button_header_search)
						.setVisibility(View.GONE);
		...
		case MODE_SEARCH:
			if (!mAdapter.isSearchMode()) {
				findViewById(R.id.btn_search_container).setVisibility(
						View.VISIBLE);
				findViewById(R.id.search_text_container).setVisibility(
						View.VISIBLE);

				findViewById(R.id.btn_edit_container).setVisibility(View.GONE);
				findViewById(R.id.btn_bottom_container)
						.setVisibility(View.GONE);
	...
	private void registerMediaStoreChangedObserver() {
		getContentResolver().registerContentObserver(
				...
				true,
				new VideoContentObserver(
						VideoContentObserver.INTERNAL_VIDEO_CONTENT_CHANGED,
						mMediaStoreChangedHandler));
		getContentResolver().registerContentObserver(
				MediaStore.Video.Media.EXTERNAL_CONTENT_URI,
				true,
				new VideoContentObserver(
						VideoContentObserver.EXTERNAL_VIDEO_CONTENT_CHANGED,
						mMediaStoreChangedHandler));
	}
	...
	public void tabIndex(int index) {
		if (mIsStartFromWheel) {
			...
			if (index <= 3 && index >= 0
					&& null != AudioPlayerService.mPlayingSong
				...
				AudioPlayerManager.getInstance().setPausedSong(storageType,
						pausedSong);
			}
		}
		switch (index) {
		...
		case TAB_INDEX_AUX:
			...
			intent_aux.setClass(AudioListActivity.this,
			...
			if (i == index) {
				...
				if (true == isZh()) {
					findViewById(TAB_IDS[i]).setBackgroundResource(
							TAB_SELECTED_BGS_ZH[i]);
				} else {
					findViewById(TAB_IDS[i]).setBackgroundResource(
							TAB_SELECTED_BGS_EN[i]);
				}
			} else {
	...
	private void startPlaySong() {
		...
		PausedSong pausedSong = AudioPlayerManager.getInstance().getPausedSong(
				viewType);
		int songPosition = 0;
		int currentTime = 0;
		if (null != pausedSong) {
	...
	public static class DeleteDialog extends DialogFragment {
		...
		public Dialog onCreateDialog(Bundle savedInstanceState) {
			...
			builder.setTitle(R.string.delete).setMessage(
			...
			builder.setPositiveButton(android.R.string.ok,
					new DialogInterface.OnClickListener() {
						...
						public void onClick(DialogInterface dialog, int which) {
							...
							AudioLoaderManager.getInstance().deleteSongs(
									mSongs, mActivity);
						}
		...
		public void handleMessage(Message msg) {
			...
			case MSG_SHOW_DELETE_PROGRESS_DIALOG:
				if (mProgressDialog == null) {
					...
					mProgressDialog.setMessage(getApplicationContext()
							.getString(R.string.deleting_song));
				}
			...
			case MSG_SHOW_SEARCH_PROGRESS_DIALOG:
				if (mProgressDialog == null) {
					...
					mProgressDialog.setMessage(getApplicationContext()
							.getString(R.string.searching_song));
				}
				if (!mProgressDialog.isShowing()) {
					mProgressDialog.show();
				}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == REQCODE_BT) {
			...
			if (true == isZh()) {
				findViewById(TAB_IDS[tabIndex]).setBackgroundResource(
						TAB_SELECTED_BGS_ZH[tabIndex]);
			} else {
				findViewById(TAB_IDS[tabIndex]).setBackgroundResource(
						TAB_SELECTED_BGS_EN[tabIndex]);
			}

			findViewById(TAB_IDS[4]).setEnabled(true);

			if (true == isZh()) {
				findViewById(TAB_IDS[4]).setBackgroundResource(
						TAB_NORMAL_BGS_ZH[4]);
			} else {
				findViewById(TAB_IDS[4]).setBackgroundResource(
						TAB_NORMAL_BGS_EN[4]);
			...
			if (true == isZh()) {
				findViewById(TAB_IDS[3]).setBackgroundResource(
						TAB_NORMAL_BGS_ZH[3]);
			} else {
				findViewById(TAB_IDS[3]).setBackgroundResource(
						TAB_NORMAL_BGS_EN[3]);
